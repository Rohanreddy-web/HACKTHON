...
async function dataFetch() {
    let data=await fetch(`http://127.0.0.1:3000/organic/`)
    ...
    for (const floder of a) {
        if(floder.href.includes("/organic")){
            let dynamicfloder = floder.href.split("/").slice(-2)[0]
         let data= await fetch(`http://127.0.0.1:3000/organic/${dynamicfloder}/info.json`)
         let resp=await data.json()
         cont.innerHTML+=`<div class="cont user-card">
    <div class="card" style="width: 18rem;">
        <img src="../organic/${dynamicfloder}/cover.jpg" class="card-img-top" alt="...">
        <div class="card-body ">
          <h5 class="card-title">${resp.Ti}</h5>
          <p class="card-text">${resp.info}</p>
          <p class="card-text bold">${resp.best1}</p>
          <div class="d-grid gap-2 col-12 mx-auto "> <a href="${resp.page}" class="btn btn-outline-success">more info</a></div>
        </div>
      </div>
</div>`
