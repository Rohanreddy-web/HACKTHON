...
async function dataFetch() {
    let data=await fetch(`http://127.0.0.1:3000/aqua/`)
    ...
    for (const floder of a) {
        if(floder.href.includes("/aqua")){
            let dynamicfloder = floder.href.split("/").slice(-2)[0]
         let data= await fetch(`http://127.0.0.1:3000/aqua/${dynamicfloder}/info.json`)
         let resp=await data.json()
         cont.innerHTML+=`<div class="cont user-card">
    <div class="card" style="width: 18rem;">
        <img src="../aqua/${dynamicfloder}/cover.jpg" class="card-img-top" alt="...">
        <div class="card-body ">
